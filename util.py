...
def collate_fn(examples):
    ...
    def merge_1d(arrays, dtype=torch.int64, pad=0):
        lengths = [len(a) for a in arrays]
        padded = torch.full((len(arrays), max(lengths)), pad, dtype=dtype)
    ...
    context_words, context_char_idxs, context_chars, \
        question_words, question_char_idxs, question_chars, \
        y1s, y2s, ids = zip(*examples)

    # Merge into batch tensors
    context_words = merge_1d(context_words)
    context_chars = merge_1d(context_chars)
    context_char_idxs = merge_1d(context_char_idxs, pad=-1)
    question_words = merge_1d(question_words)
    question_chars = merge_1d(question_chars)
    question_char_idxs = merge_1d(question_char_idxs, pad=-1)
    y1s = merge_0d(y1s)
    y2s = merge_0d(y2s)
    ids = merge_0d(ids)
    ...
    y1s += 1
    y2s += 1
    context_words = torch.cat((torch.ones(len(examples), 1, dtype=torch.int64), context_words), dim=-1)
    context_char_idxs += 1
    question_words = torch.cat((torch.ones(len(examples), 1, dtype=torch.int64), question_words), dim=-1)
    question_char_idxs += 1

    return (context_words, context_char_idxs, context_chars,
            question_words, question_char_idxs, question_chars,
            y1s, y2s, ids)
